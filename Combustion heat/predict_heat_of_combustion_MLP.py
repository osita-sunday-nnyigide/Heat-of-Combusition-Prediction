...
class DescriptorMLP(nn.Module):
    ...
    def forward(self, descriptors):
        x = F.relu(self.bn1(self.fc1(descriptors)))
        x = self.dropout(x)
...
for epoch in range(300):
    ...
    for data in train_loader:
        desc = data.descriptors.to(device)
        y = data.y.to(device)

        optimizer.zero_grad()
        out = model(desc).squeeze()
        loss = loss_fn(out, y)
        loss.backward()
        optimizer.step()
        ...
        y_denorm = y.squeeze() * label_std + label_mean
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            y_denorm = y.squeeze().cpu() * label_std + label_mean
